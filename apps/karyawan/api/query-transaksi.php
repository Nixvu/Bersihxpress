...
function getTransactions($conn, $bisnisId, $status = 'all', $search = '') {
    try {
        ...
        $sql = "
            ...
            WHERE t.bisnis_id = ? AND t.karyawan_id = ?
        ";
        $params = [$bisnisId, $karyawanId];
    ...
    try {
        $sql = "
            SELECT 
                ...
                COUNT(*) as count
            FROM transaksi 
            WHERE bisnis_id = ? AND karyawan_id = ?
            GROUP BY status
        ";
        $stmt = $conn->prepare($sql);
        $stmt->execute([$bisnisId, $karyawanId]);
...
function updateTransactionStatus($conn, $bisnisId, $data) {
    ...
    $stmt = $conn->prepare('SELECT transaksi_id, status FROM transaksi WHERE transaksi_id = ? AND bisnis_id = ? AND karyawan_id = ?');
    $stmt->execute([$transaksiId, $bisnisId, $karyawanId]);
    ...
    if ($updateFields) {
        ...
        $updateParams[] = $karyawanId;
        $stmt = $conn->prepare('UPDATE transaksi SET ' . implode(', ', $updateFields) . ' WHERE transaksi_id = ? AND bisnis_id = ? AND karyawan_id = ?');
