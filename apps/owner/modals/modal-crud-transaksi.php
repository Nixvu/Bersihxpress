...
<div id="modal-transaksi-template"
    ...
    <div class="flex-grow overflow-y-auto p-6 no-scrollbar">
        <form id="form-buat-template" action="api/query-buat-transaksi.php" method="POST" class="space-y-6">
            <!-- Hidden fields for database -->
            <input type="hidden" name="bisnis_id" id="bisnis_id_template" readonly value="<?php echo htmlspecialchars($ownerData['bisnis_id'] ?? ''); ?>">
    ...
    <script>
    // --- Modal Kirim WA Logic ---
    document.addEventListener('DOMContentLoaded', function() {
        // Template pesan WA
        const templatePesan = {
            selesai: "Halo {{nama}}, pesanan Anda dengan nota #{{no_nota}} telah selesai dan siap diambil. Terima kasih telah menggunakan layanan kami!",
            masuk: "Halo {{nama}}, pesanan Anda dengan nota #{{no_nota}} telah kami terima dan akan segera diproses.",
            pembayaran: "Halo {{nama}}, terima kasih atas pembayaran pesanan dengan nota #{{no_nota}}. Jika ada pertanyaan, silakan hubungi kami.",
            proses: "Halo {{nama}}, pesanan Anda dengan nota #{{no_nota}} sedang diproses. Kami akan mengabari jika sudah selesai."
        };

        // Fungsi untuk replace template
        function fillTemplate(template, data) {
            return template.replace(/{{nama}}/g, data.nama)
                          .replace(/{{no_nota}}/g, data.no_nota);
        }

        // Modal logic
        window.showModalKirimWA = function(transaksi) {
            // transaksi: { nama, no_nota, no_telepon }
            const modal = document.getElementById('modal-kirim-wa');
            ...
            if (!modal) return;
            phoneInput.value = transaksi.no_telepon || '';
            // Set default template
            templateSelect.value = 'selesai';
            messagePreview.value = fillTemplate(templatePesan['selesai'], transaksi);
            modal.classList.remove('hidden');
            modal.setAttribute('aria-hidden', 'false');

            // Update preview on change
            templateSelect.onchange = function() {
                messagePreview.value = fillTemplate(templatePesan[this.value], transaksi);
            };
        };

        // Button logic
        const btnKirim = document.getElementById('btn-kirim-wa-dinamis');
        btnKirim.onclick = function() {
            ...
            // Format nomor: hilangkan 0 depan, tambahkan country code 62
